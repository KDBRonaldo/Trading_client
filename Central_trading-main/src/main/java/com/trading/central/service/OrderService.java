...
public class OrderService {
    ...
    public void receiveOrder(OrderCommandMsg msg) {
        ...
        broadcaster.order(msg.getStockCode(), msg.getOrderId(), msg.getAccountId(),
                "委托已受理 " + msg.getSide() + " " + msg.getStockCode() + " " + msg.getPrice() + "x" + msg.getQuantity());
        ...
        try {
            if (matchingEngine.getCurrentPhase() == MatchingEngine.AuctionPhase.CALL_AUCTION) {
                ...
                broadcaster.order(msg.getStockCode(), msg.getOrderId(), msg.getAccountId(),
                        "已加入集合竞价池 " + msg.getSide() + " " + msg.getPrice() + "x" + msg.getQuantity());
            } else {
                // 连续竞价阶段：立即撮合
                ...
                if (result.trades.isEmpty()) {
                    ...
                    broadcaster.order(msg.getStockCode(), msg.getOrderId(), msg.getAccountId(),
                            "无匹配对手方，已挂单等待 " + msg.getSide() + " " + msg.getPrice() + "x" + msg.getQuantity());
                }
            }
        } catch (Exception e) {
